        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (vertex.length > 1 && isIndexMode) {
                ...
                if (!dictionary.getMetaWords().contains(vertex.getWord())) {
                    ...
                    // 子图节点数量 > 2 表示还有子词可以生成
                    if (enableSingleWord || subGraph.vertexes.length > 2) {
                        ...
                        int vertexEnd = vertex.index + vertex.length; // 父词结束位置

                        for (Term subTerm : subTerms) {
                            int termLength = subTerm.getEnd() - subTerm.getOffset();
                            if (enableSingleWord || termLength > 1 || (dictionary.getDoubleArrayTrie().get(subTerm.getText()) != null)) {
